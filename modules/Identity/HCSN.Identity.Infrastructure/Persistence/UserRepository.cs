...
public class UserRepository : IUserRepository
{
    ...
    {
        return await _context
            .Users.Where(u => u.AccessibleSystems.Contains(systemName) && u.IsActive)
            .Include(u => u.Tenant)
            .ToListAsync();
    }
    ...
    {
        return await _context
            .Users.Where(u => u.TenantId == tenantId && u.Email == email)
            .Include(u => u.Tenant)
            .FirstOrDefaultAsync();
    }
    ...
    {
        return await _context
            .Users.Where(u => u.TenantId == tenantId && u.IsTenantAdmin && u.IsActive)
            .Include(u => u.Tenant)
            .ToListAsync();
    }
    ...
    {
        return await _context
            .Users.Where(u => u.TenantId == tenantId && u.Status == UserStatus.PendingApproval)
            .Include(u => u.Tenant)
            .ToListAsync();
    }
}
