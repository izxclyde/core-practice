...
public class TenantRegistrationService : ITenantRegistrationService, ITenantAdminService
{
    ...
    public async Task<RegistrationResult> RegisterAsync(TenantRegisterRequest request)
    {
        ...
        if (tenant.AllowedDomains?.Any() == true)
        {
            ...
            if (!tenant.IsDomainAllowed(emailDomain))
            {
                return new RegistrationResult
                {
                    Success = false,
                    Message =
                        $"Email domain must be one of: {string.Join(", ", tenant.AllowedDomains)}",
        ...
        if (customFieldsData.Any())
        {
            user.SetMetadata(
                "custom_fields",
                System.Text.Json.JsonSerializer.Serialize(customFieldsData)
            );
        }
    ...
    public async Task<RegistrationSettingsDto> GetRegistrationSettingsAsync(string identifier)
    {
        ...
        return new RegistrationSettingsDto
        {
            ...
            CollectPhoneNumber =
                settings.CustomFields?.Any(f => f.FieldName == "PhoneNumber") ?? false,
            CollectAddress = settings.CustomFields?.Any(f => f.FieldName == "Address") ?? false,
            CollectCompanyName =
                settings.CustomFields?.Any(f => f.FieldName == "CompanyName") ?? false,

            // New flexible fields
            CustomFields =
                settings
                    .CustomFields?.Select(f => new CustomFieldDto
                    {
    ...
    public async Task<RegistrationResult> RegisterWithInviteAsync(
        string token,
    ...
    public async Task<List<string>> GetRequiredFieldsAsync(string identifier)
    {
        ...
        return tenant
            .Settings.Registration.CustomFields.Where(f => f.IsRequired)
            .Select(f => f.FieldName)
            .ToList();
    }

    public async Task<RegistrationResult> ApproveUserAsync(
        Guid userId,
        Guid tenantId,
        string? notes = null
    )
    ...
    public async Task<List<PendingUserDto>> GetPendingApprovalsAsync(Guid tenantId)
    {
        ...
        return pendingUsers
            .Select(u => new PendingUserDto(
                ...
                u.FullName,
                u.CreatedAt,
                u.GetMetadata<Dictionary<string, object>>("custom_fields")
            ...
            switch (field.FieldName.ToLower())
            {
                case "email":
                    if (string.IsNullOrEmpty(request.Email))
                        ...
                        !string.IsNullOrEmpty(field.ValidationRegex)
                ...
                default:
                    ...
                    if (
                        ...
                        || !request.CustomFields.ContainsKey(field.FieldName)
                    )
                    {
                        errors[field.FieldName] =
                            field.ErrorMessage ?? $"{field.FieldName} is required";
                    }
                    else if (
                        !string.IsNullOrEmpty(field.ValidationRegex)
                        && !Regex.IsMatch(
                            request.CustomFields[field.FieldName]?.ToString() ?? "",
                            field.ValidationRegex
                        )
                    )
                    {
                        errors[field.FieldName] =
                            field.ErrorMessage ?? $"Invalid {field.FieldName} format";
                    }
                    break;
            }
        }
        ...
        foreach (var field in customFields)
        {
            if (requestCustomFields?.TryGetValue(field.FieldName, out var value) == true)
            {
                ...
                if (
                    ...
                    && field.Options?.Contains(value?.ToString() ?? "") == false
                )
                {
                    _logger.LogWarning(
                        "Invalid option for field {FieldName}: {Value}",
                        field.FieldName,
                        value
                    );
                    continue;
    ...
    private List<string> DetermineUserAccessibleSystems(
        Tenant tenant,
        TenantRegisterRequest request
    )
    {
    ...
    private async Task NotifyAdminsOfPendingUser(
        User user,
        Tenant tenant,
    ...
    )
    {
        ...
        var customFieldsHtml = string.Join(
            "<br/>",
            customFields.Select(f => $"{f.Key}: {f.Value}")
        );
        ...
        foreach (var admin in admins)
        {
            ...
            {
                _logger.LogWarning(
                    "Email service not configured. Would have sent approval request to {Email} for user {UserId}",
                    admin.Email,
                    user.Id
                );
            }
    ...
    private async Task SendConfirmationEmail(User user, string token, Tenant tenant)
    {
        ...
        {
            _logger.LogWarning(
                "Email service not configured. Would have sent confirmation email to {Email} with token {Token}",
                user.Email,
                token
            );
        }
    ...
    private async Task SendApprovalEmail(User user, Tenant tenant, string token)
    {
        ...
        {
            _logger.LogWarning(
                "Email service not configured. Would have sent approval email to {Email}",
                user.Email
            );
        }
    }
    ...
    private async Task SendRejectionEmail(User user, string reason)
    {
        ...
        {
            _logger.LogWarning(
                "Email service not configured. Would have sent rejection email to {Email} with reason: {Reason}",
                user.Email,
                reason
            );
        }
    ...
    private async Task AutoProvisionUserWorkspace(User user, Tenant tenant)
    {
        ...
        _logger.LogInformation(
            "Auto-provisioning workspace for user {UserId} in tenant {TenantId}",
            user.Id,
            tenant.Id
        );
