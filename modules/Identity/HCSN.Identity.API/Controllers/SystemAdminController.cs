...
public class SystemAdminController : ControllerBase
{
    ...
    public async Task<ActionResult<TenantDto>> CreateTenant(CreateTenantWithSettingsRequest request)
    {
        ...
        {
            return BadRequest(
                new { error = "Subdomain already exists", code = "SUBDOMAIN_EXISTS" }
            );
        }
        ...
        {
            var existingCustom = await _tenantRepository.GetByCustomDomainAsync(
                request.CustomDomain
            );
            if (existingCustom != null)
            {
                return BadRequest(
                    new { error = "Custom domain already exists", code = "CUSTOM_DOMAIN_EXISTS" }
                );
            }
        }
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Tenant created: {TenantName} ({TenantId}) by {User}",
                tenant.Name,
                tenant.Id,
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(
                500,
                new { error = "Error creating tenant", code = "CREATION_FAILED" }
            );
        }
    }
    ...
    public async Task<IActionResult> EnableFeature(
        Guid id,
        string feature,
        [FromBody] object? configuration = null
    )
    {
        if (!await IsSystemAdmin())
            return Forbid("SystemAdmin access required");
    ...
    public async Task<ActionResult<object>> GetTenantStats()
    {
        ...
        var stats = new
        {
            ...
            byStatus = tenantsList
                .GroupBy(t => t.Status)
                .ToDictionary(g => g.Key.ToString(), g => g.Count()),
            byDeploymentModel = tenantsList
                .GroupBy(t => t.DeploymentModel)
                .ToDictionary(g => g.Key.ToString(), g => g.Count()),
            ...
            dedicated = tenantsList.Count(t =>
                t.DeploymentModel == TenantDeploymentModel.Dedicated
            ),
